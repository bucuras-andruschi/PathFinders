...
void setup() {
  ...
  digitalWrite(S1, LOW);
 //  moveRobot(255, 255); // Adjust speed values according to your robot's dynamics for going forward
}
...
void checkColorSensor() {
  ...
  // Check for blue or yellow color
  if (blue < red && blue < green && blue < 0 && blue > -25) // ||(red >= -20 && red <= -10 && blue >= -20 && blue <= -10 && green >= -20 && green <= -10))
   {
    // Detected blue or yellow color
    Serial.println("Detected color: Blue or Yellow");
    servoMotor.write(120); // Turn left
     moveRobot(255, 255);
        delay(1800); // Wait for 300 ms
  } else {
    // Detected any other color
    Serial.println("Detected color: Not Blue or Yellow");
    servoMotor.write(90); // Go straight
    moveRobot(255, 255);
  }
}
